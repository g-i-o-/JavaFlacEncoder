...
/**
 * EncodedElement class provides is used to store data in the proper bitstream format for FLAC
 * audio. Methods are provided to easily append values to the bitstream. Conceptually, an
 * EncodedElement is a list structure, in which the encoded data is stored in a byte array. It is
 * assumed that any data stored by objects of this class will be retrieved through direct access to
 * the underlying byte array. This byte array is exposed to outside objects primarily to allow
 * faster access than "proper" object-oriented design might allow.
...
public class EncodedElement {
	...
	/**
	 * Previous element in list. At current times, this should not be dependend on to be set
	 */
	...
	/**
	 * Data stored by this element. Member usableBits should be used to track the last valid index
	 * at a bit level.
	 */
	byte[] data;

	/**
	 * Use to track the last valid index of the data array at a bit level. For example, a value of
	 * "10" would mean the first byte and two low-order bits of the second byte are used. usableBits
	 * must always be equal or greater than offset.
	 */
	int usableBits; // i.e, the last write location in 'data' array.

	/**
	 * Used to signify the index of the first valid bit of the data array. For purposes of speed, it
	 * is not always best to pack the data starting at bit zero of first byte. offset must always be
	 * less than or equal to usableBits.
	 */
	protected int offset;

	/**
	 * Constructor, creates an empty element with offset of zero and array size of 100. This array
	 * can be replaced with a call to setData(...).
	 */
	 ...
	 * @param array
	 *            Byte array to use for this element.
	 * @param off
	 *            Offset to use for this element. Usablebits will also be set to this value.
	 */
	 ...
	 * Constructor. Creates an EncodedElement with the given size and offset
	 * @param size
	 *            Size of data array to use(in bytes)
	 * @param off
	 *            Offset to use for this element. Usablebits will also be set to this value.
	 */
	 ...
	 * @param ele
	 *            the object to set as previous.
	 * @return <code>void</code> Precondition: none Post-condition: getPrevious() will now return
	 ...
	 * @return void Pre-condition: none Post-condition: getNext() will now return the given object.
	...
	/**
	 * Set the number of bits of the given array that are usable data. Data is packed from the lower
	 * indices to higher.
	 * @param bits
	 *            the value to store
	 */
	...
	/**
	 * Return the last element of the list given. This is a static funtion to provide minor speed
	 * improvement. Loops through all elements' "next" pointers, till the last is found.
	 * @param e
	...
	/**
	 * Return the last element of the list given. Loops through all elements' "next" pointers, till
	 * the last is found.
	...
	/**
	 * Add a number of bits from a long to the end of this list's data. Will add a new element if
	 * necessary. The bits stored are taken from the lower- order of input.
	 * @param input
	 *            Long containing bits to append to end.
	 * @param bitCount
	...
	/**
	 * Add a number of bits from an int to the end of this list's data. Will add a new element if
	 * necessary. The bits stored are taken from the lower- order of input.
	 * @param input
	 *            Int containing bits to append to end.
	 * @param bitCount
	 *            Number of bits to append.
	 * @return EncodedElement which actually contains the appended value.
	 */
	public EncodedElement addInt(int input, int bitCount) {
		if (next != null) {
			...
			next = new EncodedElement(size, tOff);
			System.err.println("creating next node of size:bitCount " + size + ":" + bitCount + ":"
					+ usableBits + ":" + data.length);
			...
			/* if(startPos/8+8 > dest.length) EncodedElement.addInt(input, bitCount, startPos,
			 * dest); else EncodedElement.addInt_new(input, bitCount, startPos, dest); */
			usableBits += bitCount;
			return this;
		}
	}
	...
	/**
	 * Append an equal number of bits from each int in an array within given limits to the end of
	 * this list.
	 * @param inputArray
	 *            Array storing input values.
	 * @param bitSize
	 *            number of bits to store from each value.
	 * @param start
	 *            index of first usable index.
	 * @param skip
	 *            number of indices to skip between values(in case input data is interleaved with
	 *            non-desirable data).
	 * @param countA
	 *            Number of total indices to store from.
	 * @return EncodedElement containing end of packed data. Data may flow between multiple
	 *         EncodedElement's, if an existing element was not large enough for all values.
	 */
	 ...
	 * @param inputBits
	 *            Array containing number of bits to use for each index packed. This array should be
	 *            equal in size to the inputA array.
	 * @param inputOffset
	 *            Index of first usable index.
	 * @param countA
	...
	public EncodedElement packIntByBits(int[] inputA, int[] inputBits, int inputOffset, int countA) {
		...
		// pack them and update usable bits.
		if (writeCount > 0) {
			EncodedElement.packIntByBits(inputA, inputBits, inputOffset, writeCount, usableBits,
					data);
	...
	/**
	 * Total number of usable bits stored by this entire list. This sums the difference of each list
	 * element's "usableBits" and "offset".
	...
	private static void addInt_new(int value, int count, int startPos, byte[] dest) {
		/*
		 * if(startPos/8+4 > buf.capacity()) { addInt(value, count, startPos, dest); return; } */
		int secondInt = 0;
		int secondCount = 0;
		start: {
			...
			int workingIntCache = dest[index++] << 24 | dest[index++] << 16 | dest[index++] << 8
					| dest[index];
			...
			if (!onIndex) {
				...
				workingInt = workingInt & 0xFFFFFFFF << startPos % 8;// clear lower bits
				workingInt = workingInt | value;
				value = workingInt >>> 32 - (startPos % 8 + count);
				count = count + startPos % 8;
			}
			value = value << 32 - count;// place into upper bits, we fill from top
			// int workingInt = buf.getInt(startPos/8);
			int workingInt = workingIntCache;
	 ...
	 * @param value
	 *            int to store bits from
	 * @param count
	 *            number of low-order bits to store
	 * @param startPos
	 *            start bit location in array to begin writing
	 * @param dest
	 *            array to store bits in. dest MUST have enough space to store the given data, or
	...
	/**
	 * public static void addIntOld(int input, int count, int startPos, byte[] dest) { if(DEBUG_LEV
	 * > 30) System.err.println("EncodedElement::addInt : Begin"); int currentByte = startPos/8; int
	 * currentOffset = startPos%8; int bitRoom;//how many bits can be placed in current byte int
	 * upMask;//to clear upper bits(lower bits auto-cleared by L-shift int downShift;//bits to shift
	 * down, isolating top bits of input int upShift;//bits to shift up, packing byte from top.
	 * while(count > 0) { //find how many bits can be placed in current byte bitRoom =
	 * 8-currentOffset; //get those bits //i.e, take upper 'bitsNeeded' of input, put to lower part
	 * of byte. downShift = count-bitRoom; upMask = 255 >>> currentOffset; upShift = 0; if(downShift
	 * < 0) { //upMask = 255 >>> bitRoom-count; upShift = bitRoom - count; upMask = 255 >>>
	 * (currentOffset+upShift); downShift = 0; } if(DEBUG_LEV > 30) {
	 * System.err.println("count:offset:bitRoom:downShift:upShift:" +
	 * count+":"+currentOffset+":"+bitRoom+":"+downShift+":"+upShift); } int currentBits = (input
	 * >>> downShift) & (upMask); //shift bits back up to match offset currentBits = currentBits <<
	 * upShift; upMask = (byte)upMask << upShift; dest[currentByte] = (byte)(dest[currentByte] &
	 * (~upMask)); //merge bytes~ dest[currentByte] = (byte)(dest[currentByte] | currentBits);
	 * //System.out.println("new currentByte: " + dest[currentByte]); count -= bitRoom;
	 * currentOffset = 0; currentByte++; } if(DEBUG_LEV > 30)
	...
	public static void addLong(long input, int count, int startPos, byte[] dest) {
		...
		while (count > 0) {
			...
			if (DEBUG_LEV > 30) {
				System.err.println("count:offset:bitRoom:downShift:upShift:" + count + ":"
						+ currentOffset + ":" + bitRoom + ":" + downShift + ":" + upShift);
			}
	...
	/**
	 * public static void packIntOLD_WORKING(int[] inputArray, int startBitSize, int startPos, int
	 * start, int skip, int count, byte[] dest) { if(DEBUG_LEV > 0)
	 * System.err.println("EncodedElement::packInt : Begin"); if(DEBUG_LEV > 10)
	 * System.err.println("start:skip:count : " +start+":"+skip+":"+count); for(int i = 0; i <
	 * count; i++) { addInt(inputArray[i*(skip+1)+start], startBitSize, startPos, dest);
	 * startPos+=startBitSize; } }
	 **/

	/**
	 * Append an equal number of bits from each int in an array within given limits to the given
	 * byte array.
	 * @param inputArray
	 *            Array storing input values.
	 * @param bitSize
	 ...
	 *            Number of total indices to store from.
	 * @param startPosIn
	 *            First usable index in destination array(byte index = startPosIn/8, bit within that
	 *            byte = startPosIn%8)
	 * @param dest
	 *            Destination array to store input values in. This array *must* be large enough to
	 *            store all values or this method will fail in an undefined manner.
	 */
	public static void packInt(int[] inputArray, int bitSize, int startPosIn, int start, int skip,
			int countA, byte[] dest) {
		if (DEBUG_LEV > 30)
	 ...
	 *            Number of indices to pack.
	 * @param startPosIn
	 *            First usable bit-level index in destination array(byte index = startPosIn/8, bit
	 *            within that byte = startPosIn%8)
	 * @param dest
	 *            Destination array to store input values in. This array *must* be large enough to
	 *            store all values or this method will fail in an undefined manner.
	 */
	private static void packIntByBits_new(int[] inputA, int[] inputBits, int inputOffset,
		...
		for (int valI = inputOffset; valI < inputStop; valI++) {
			...
			if (startPos / 8 + 8 > dest.length)
				...
				start: {
					...
					int count2 = startPos % 8 + count - 32;
					if (count2 > 0) {
						// addInt_buf2(value,count2,startPos+32, dest);//split low-order and recurse
						value = value >>> count2;// shift high-order down
						secondInt = value;
						secondCount = count2;
						count -= count2;
			...
			startPos += inCount;// startPos must not be referenced again below here!
	 ...
	 *            store all values or this method will fail in an undefined manner.
	 */
	public static void packIntByBits(int[] inputA, int[] inputBits, int inputOffset, int countA,
			int startPosIn, byte[] dest) {
		if (DEBUG_LEV > 30)
			System.err.println("EncodedElement::packIntByBits : Begin");
		// int offsetCounter = 0;
		int startPos = startPosIn;// the position to write to in output array
	...
	/**
	 * Force the usable data stored in this list ends on a a byte boundary, by padding to the end
	 * with zeros.
	 * @return true if the data was padded, false if it already ended on a byte boundary.
	 */
	...
	public short getCRC16() {
		...
		if (nextEl != null) {
			byte partial = usableBits % 8 == 0 ? 0 : input[stop];
			// byte partial = input[stop];
			if (usableBits % 8 != 0)
				System.err.println("UsableBits%8 == " + usableBits % 8);
			nextEl.getCRC16(partial, usableBits % 8, crc);
		}
	...
	private void getCRC16(byte leadByte, int bitCount, CRC16 crc) {
		...
		EncodedElement nextEl = getNext();
		if (nextEl != null) {
			byte partial = usableBits % 8 == 0 ? 0 : data[stop];
			nextEl.getCRC16(partial, usableBits % 8, crc);
		}
